...
export class Delve {
	...
	constructor(launchArgs: LaunchRequestArguments, target: string) {
		...
		this.connection = new Promise(async (resolve, reject) => {
			...
			if (!target) {
				return reject('The target attribute is missing in the debug configuration in launch.json');
			}
			...
			if (!existsSync(launchArgs.plzBinPath)) {
				return reject('Cannot find Please. Install from https://github.com/thought-machine/please and ensure it is in the "PATH" environment variable.');
			}
	...
	public async getDebugState(): Promise<DebuggerState> {
		...
		const callResult = await this.callPromise<CommandOut>('State', [{ NonBlocking: true }]);
...
export class GoDebugSession extends LoggingDebugSession {
	...
	protected attachRequest(response: DebugProtocol.AttachResponse, args: unknown): void {
		this.sendErrorResponse(
			response,
			3000,
	...
	protected toLocalPath(pathToConvert: string): string {
		...
		if (!substituteRule) {
			...
			const index = pathToConvert.indexOf(`${this.pathSeparator}src${this.pathSeparator}`);
			...
			if (gopath && indexGoModCache > 0) {
				return path.join(
					gopath,
					pathToConvert.substr(indexGoModCache)
				);
			}
		}
	...
	protected async stackTraceRequest(
		...
		const stackTraceIn = { id: goroutineId, depth: this.delve.stackTraceDepth, full: false, cfg: this.delve.loadConfig };
								...
								if (err.toString().startsWith('Breakpoint exists at')) {
									...
									if (!existingBreakpoints) {
										try {
											...
											>('ListBreakpoints', [{}]);
											existingBreakpoints = (listBreakpointsResponse as ListBreakpointsOut).Breakpoints;
										} catch (error) {
