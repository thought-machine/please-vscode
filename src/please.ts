...
export async function version(): Promise<string> {
    return await new Promise((resolve, reject) => {
        execFile(binPath(), ['--version'], { cwd: repoRoot() || path.resolve('.') }, (err, stdout) => {
            if (err) {
                return reject(err);
            }
...
export async function getInputTargets(filename: string): Promise<Array<string>> {
    ...
    return await new Promise((resolve, reject) => {
        execFile(binPath(), ['query', 'whatinputs', filename], { cwd: repoPath }, (err, stdout) => {
            if (err) {
                return reject(err);
            }

            const targets = stdout.split('\n').filter(Boolean);
            if (!targets.every((target) => target.startsWith(':') || target.startsWith('//'))) {
                return reject(new Error(`No targets found with file as source: ${filename}`));
            }
